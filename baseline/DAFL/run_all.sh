...
TARGETS=("cflow")
TIMELIMIT=180 # Timeout in seconds
REPEAT=3
...
# Create a temporary container to check directories in container
TEMP_CONTAINER="dafl_temp_$(date +%s)"
docker run -dit --name "$TEMP_CONTAINER" "$IMAGE_NAME" bash > /dev/null 2>&1
log_msg "Created temporary container to check directories"
...
for ((r=1; r<=REPEAT; r++)); do
  for target in "${TARGETS[@]}"; do
    ...
    SUBDIRS=$(docker exec "$TEMP_CONTAINER" find "$VARIANT_DIR" -maxdepth 1 -mindepth 1 -type d)
    ...
    for binary_dir in $SUBDIRS; do
      {
        ...
        if ! docker exec "$TEMP_CONTAINER" test -f "$BINARY_PATH"; then
        ...
        case "$target" in
            ...
            "lame")    ARGS="@@ /dev/null" ;;
            "jhead")   ARGS="@@" ;;
            *) log_msg "Unknown target $target"; continue ;;
        esac
...
log_msg "Removed temporary container"
...
log_msg "All fuzzing tasks completed successfully"
