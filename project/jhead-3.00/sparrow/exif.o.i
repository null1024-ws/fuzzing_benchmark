# 1 "/benchmark/project/jhead-3.00/exif.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/project/jhead-3.00/exif.c" 2
...
extern int ShowTags;
# 10 "/benchmark/project/jhead-3.00/exif.c" 2
...
# 12 "/benchmark/project/jhead-3.00/exif.c" 2
...
# 63 "/benchmark/project/jhead-3.00/exif.c"
...
# 189 "/benchmark/project/jhead-3.00/exif.c"
static const TagTable_t TagTable[] = {
  ...
  { 0x9c9b, "Windows-XP Title"},
  { 0x9c9c, "Windows-XP comment"},
  { 0x9c9d, "Windows-XP author"},
  { 0x9c9e, "Windows-XP keywords"},
  { 0x9c9f, "Windows-XP subject"},
...
void PrintFormatNumber(void * ValuePtr, int Format, int ByteCount)
{
    ...
    for(n=0;n<16;n++){
        switch(Format){
            case 6:
            case 1: printf("%02x",*(uchar *)ValuePtr); s=1; break;
            case 3: printf("%d",Get16u(ValuePtr)); s=2; break;
            case 4:
            case 9: printf("%d",Get32s(ValuePtr)); s=4; break;
            case 8: printf("%hd",(signed short)Get16u(ValuePtr)); s=2; break;
            case 5:
            ...
            case 11: printf("%f",(double)*(float *)ValuePtr); s=8; break;
            case 12: printf("%f",*(double *)ValuePtr); s=8; break;
            default:
...
double ConvertAnyFormat(void * ValuePtr, int Format)
{
    ...
    switch(Format){
        case 6: Value = *(signed char *)ValuePtr; break;
        case 1: Value = *(uchar *)ValuePtr; break;

        case 3: Value = Get16u(ValuePtr); break;
        case 4: Value = Get32u(ValuePtr); break;
        ...
        case 8: Value = (signed short)Get16u(ValuePtr); break;
        case 9: Value = Get32s(ValuePtr); break;


        case 11: Value = (double)*(float *)ValuePtr; break;
        case 12: Value = *(double *)ValuePtr; break;
        ...
        unsigned ExifLength, int NestingLevel)
{
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        if (ShowTags){
            ...
            for (a=0;;a++){
                if (a >= (sizeof(TagTable) / sizeof(TagTable_t))){
        ...
        switch(Tag){
            ...
            case 0x9004:
            case 0x0132:
                if (!((*__ctype_b_loc ())[(int) ((ImageInfo.DateTime[0]))] & (unsigned short int) _ISdigit)){
            ...
            case 0x9c9c:
                ...
                if (ByteCount > 1){
                    if (ByteCount > 16000) ByteCount = 16000;
            ...
            case 0xA005:
                if (Tag == 0xA005 && ShowTags) printf("%s    Interop Dir:",IndentString);
                {
...

    {
        ...
        if ((DirStart+2+12*(NumDirEntries)) + 4 <= OffsetBase+ExifLength){
...
void process_EXIF (unsigned char * ExifSection, unsigned int length)
{
    ...
    DirWithThumbnailPtrs = ((void*)0);



    ProcessExifDir(ExifSection+8+FirstOffset, ExifSection+8, length-8, 0);

    ImageInfo.ThumbnailAtEnd = ImageInfo.ThumbnailOffset >= ImageInfo.LargestExifOffset ? 1 : 0;
...
void create_EXIF(void)
{
    ...
    DataWriteIndex = 16;
    Put32u(Buffer+12, DataWriteIndex-8);

    {
        ...
        Put16u(Buffer+DirIndex, NumEntries);
        DirIndex += 2;


        {

            Put16u(Buffer+DirIndex, 0x0132);
            Put16u(Buffer+DirIndex + 2, 2);
            Put32u(Buffer+DirIndex + 4, 20);
            ...
            Put16u(Buffer+DirIndex, 0x8769);
            Put16u(Buffer+DirIndex + 2, 4);
...

    {
...

        {
            ...
            Put16u(Buffer+DirIndex, 0x0202);
            Put16u(Buffer+DirIndex + 2, 4);
            Put32u(Buffer+DirIndex + 4, 1);
            Put32u(Buffer+DirIndex + 8, 0);
            DirIndex += 12;
        }
...

    {
        ...
        CreateSection(0xE1, NewBuf, DataWriteIndex);
...
{
    int a;
    if (NumOrientations == 0) return ((void*)0);
...
void ShowImageInfo(int ShowFileInfo)
{
    ...
    if (ImageInfo.Process != 0xC0){
        ...
        unsigned a;
        for (a=0;;a++){
            if (a >= (sizeof(ProcessTable) / sizeof(TagTable_t))){
