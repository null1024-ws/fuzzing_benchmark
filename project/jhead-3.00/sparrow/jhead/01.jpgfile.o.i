# 1 "/benchmark/project/jhead-3.00/jpgfile.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/project/jhead-3.00/jpgfile.c" 2
...
extern int ShowTags;
# 10 "/benchmark/project/jhead-3.00/jpgfile.c" 2


ImageInfo_t ImageInfo;


static Section_t * Sections = ((void*)0);
...
int ReadJpegSections (FILE * infile, ReadMode_t ReadMode)
{
    ...
    for(;;){
        ...
        got = fread(Data+2, 1, itemlen-2, infile);
        if (got != itemlen-2){
        ...
        switch(marker){
            ...
            case 0xDA:
                ...
                if (ReadMode & READ_IMAGE){
                    ...
                    Sections[SectionsRead].Type = 0x123;;
                    SectionsRead ++;
                    HaveAll = 1;
                }
                return 1;
            ...
            case 0xE0:
                ...
                ImageInfo.JfifHeader.Present = 1;
            ...
            case 0xE1:
                ...
                if (ReadMode & READ_METADATA){
                    if (memcmp(Data+2, "Exif", 4) == 0){
                        ...
                        Sections[SectionsRead-1].Type = 0x10E1;
                        if (ShowTags){
...
int ReplaceThumbnail(const char * ThumbFileName)
{
    ...
    if (ThumbFileName){
        ...
        if (ThumbnailFile == ((void*)0)){
            ErrFatal("Could not read thumbnail file");
            return 0;
        }


        fseek(ThumbnailFile, 0, 2);

        ThumbLen = ftell(ThumbnailFile);
        fseek(ThumbnailFile, 0, 0);
...
void DiscardAllButExif(void)
{
    ...
    for (a=0;a<SectionsRead;a++){
        if (Sections[a].Type == 0xE1 && ExifKeeper.Type == 0){
           ExifKeeper = Sections[a];
        }else if (Sections[a].Type == 0x10E1 && XmpKeeper.Type == 0){
           XmpKeeper = Sections[a];
        }else if (Sections[a].Type == 0xFE && CommentKeeper.Type == 0){
            CommentKeeper = Sections[a];
        }else if (Sections[a].Type == 0xED && IptcKeeper.Type == 0){
...
void WriteJpegFile(const char * FileName)
{
    ...
    if (Sections[0].Type != 0xE1 && Sections[0].Type != 0xE0){
...
Section_t * CreateSection(int SectionType, unsigned char * Data, int Size)
{
    ...
    if (SectionType == 0xE1){
        ...
        for (;NewIndex < 3;NewIndex++){
            if (Sections[NewIndex].Type == 0xE0) continue;
            if (Sections[NewIndex].Type == 0xE1) continue;
            break;
        }
    }
