# 1 "/benchmark/project/cflow-1.6/src/output.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/project/cflow-1.6/src/output.c" 2
# 17 "/benchmark/project/cflow-1.6/src/output.c"
# 1 "./cflow.h" 1
# 18 "./cflow.h"
# 1 "../config.h" 1
# 19 "./cflow.h" 2
...
# 18 "/benchmark/project/cflow-1.6/src/output.c" 2
# 1 "./parser.h" 1
# 18 "./parser.h"
enum {
     LBRACE = '{',
...
# 19 "/benchmark/project/cflow-1.6/src/output.c" 2
...
# 156 "/benchmark/project/cflow-1.6/src/output.c"
static int
...
{
     return p != ((void*)0) && include_symbol((Symbol*)p->data);
}
...
direct_tree(int lev, int last, Symbol *sym)
{
  ...
  || !include_symbol(sym))
   ...
   return;
     set_active(sym);
     for (p = ((sym->callee) ? (sym->callee)->head : ((void*)0)); p; p = p->next) {
...
inverted_tree(int lev, int last, Symbol *sym)
{
  ...
  || !include_symbol(sym))
   ...
   return;
     set_active(sym);
     for (p = ((sym->caller) ? (sym->caller)->head : ((void*)0)); p; p = p->next) {
   set_level_mark(lev+1, !is_last(p));
...
tree_output()
{
   ...
   if (symbols[i]->callee) {
        ...
        for (p = ((symbols[i]->callee) ? (symbols[i]->callee)->head : ((void*)0)); p;
      p = p->next) {
   ...
   if ((main_sym = start_name ? lookup(start_name) : ((void*)0)) != ((void*)0)) {
   ...
   && symbols[i]->source
   && (all_functions > 1 || symbols[i]->caller == ((void*)0))) {
...
output()
{
   ...
   if (!outfile)
        error(1, (*__errno_location ()), dcgettext (((void*)0), "cannot open file `%s'", 5), outname);
     }

     set_level_mark(0, 0);
     if (print_option & 0x01) {
   xref_output();
     }
     if (print_option & 0x02) {
   tree_output();
     }
     fclose(outfile);
}
