# 1 "/benchmark/project/cflow-1.6/src/parser.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/project/cflow-1.6/src/parser.c" 2
# 17 "/benchmark/project/cflow-1.6/src/parser.c"
# 1 "./cflow.h" 1
# 18 "./cflow.h"
# 1 "../config.h" 1
# 19 "./cflow.h" 2
...
# 18 "/benchmark/project/cflow-1.6/src/parser.c" 2
# 1 "./parser.h" 1
# 18 "./parser.h"
enum {
     LBRACE = '{',
...
# 19 "/benchmark/project/cflow-1.6/src/parser.c" 2
...
# 20 "/benchmark/project/cflow-1.6/src/parser.c" 2
...
token_type_str(int t)
{
   ...
   return "QUALIFIER";
     ...
     if (((*__ctype_b_loc ())[(int) ((t))] & (unsigned short int) _ISprint))
...
file_error(char *msg, TOKSTK *tokptr)
{
    ...
    if (tokptr) {
 fprintf(stderr, dcgettext (((void*)0), " near ", 5));
 print_token(tokptr);
    }
    fprintf(stderr, "\n");
}
...
{
     if (curs == 0) {
   error(0, 0, dcgettext (((void*)0), "INTERNAL ERROR: cannot return token to stream", 5));
   abort();
     }
     curs--;
     if (curs > 0) {
...
init_parse()
{
     _obstack_begin ((&text_stk), 0, 0, (xmalloc), (free));
...
save_token(TOKSTK *tokptr)
{
     ...
     case QUALIFIER:
   if (need_space)
        __extension__ ({ struct obstack *__o = (&text_stk); if (__extension__ ({ struct obstack const *__o1 = (__o); (size_t) (__o1->chunk_limit - __o1->next_free); }) < 1) _obstack_newchunk (__o, 1); ((void) (*((__o)->next_free)++ = (' '))); });
   len = strlen(tokptr->token);
   __extension__ ({ struct obstack *__o = (&text_stk); size_t __len = (len); if (__extension__ ({ struct obstack const *__o1 = (__o); (size_t) (__o1->chunk_limit - __o1->next_free); }) < __len) _obstack_newchunk (__o, __len); memcpy (__o->next_free, tokptr->token, __len); __o->next_free += __len; (void) 0; });
   need_space = 1;
   break;
     case MODIFIER:
   if (need_space)
   ...
   if (need_space)
        __extension__ ({ struct obstack *__o = (&text_stk); if (__extension__ ({ struct obstack const *__o1 = (__o); (size_t) (__o1->chunk_limit - __o1->next_free); }) < 1) _obstack_newchunk (__o, 1); ((void) (*((__o)->next_free)++ = (' '))); });
   __extension__ ({ struct obstack *__o = (&text_stk); if (__extension__ ({ struct obstack const *__o1 = (__o); (size_t) (__o1->chunk_limit - __o1->next_free); }) < 1) _obstack_newchunk (__o, 1); ((void) (*((__o)->next_free)++ = (tokptr->type))); });
   need_space = 0;
   break;
     case ')':
   __extension__ ({ struct obstack *__o = (&text_stk); if (__extension__ ({ struct obstack const *__o1 = (__o); (size_t) (__o1->chunk_limit - __o1->next_free); }) < 1) _obstack_newchunk (__o, 1); ((void) (*((__o)->next_free)++ = (tokptr->type))); });
   ...
   break;
     case OP:
   __extension__ ({ struct obstack *__o = (&text_stk); if (__extension__ ({ struct obstack const *__o1 = (__o); (size_t) (__o1->chunk_limit - __o1->next_free); }) < 1) _obstack_newchunk (__o, 1); ((void) (*((__o)->next_free)++ = (' '))); });
   __extension__ ({ struct obstack *__o = (&text_stk); size_t __len = (strlen(tokptr->token)); if (__extension__ ({ struct obstack const *__o1 = (__o); (size_t) (__o1->chunk_limit - __o1->next_free); }) < __len) _obstack_newchunk (__o, __len); memcpy (__o->next_free, tokptr->token, __len); __o->next_free += __len; (void) 0; });
   need_space = 1;
   break;
     default:
   if (verbose)
        file_error(dcgettext (((void*)0), "unrecognized definition", 5), tokptr);
     }
}
...
is_function()
{
   ...
   case PARM_WRAPPER:
      ...
      file_error(dcgettext (((void*)0), "unexpected end of file in declaration", 5),
          ((void*)0));
        continue;
   case '(':
...
expression()
{
   ...
   case 0:
        if (verbose)
      file_error(dcgettext (((void*)0), "unexpected end of file in expression", 5),
          ((void*)0));
        return;
...
parse_function_declaration(Ident *ident, int parm)
{
 ...
 restart:
   ...
   if (error_recovery)
        ...
        if (verbose)
      file_error(dcgettext (((void*)0), "expected `;'", 5), &tok);
        error_recovery = 1;
   }
   goto restart;
   ...
   break;
     case 0:
   if (verbose)
        file_error(dcgettext (((void*)0), "unexpected end of file in declaration", 5), ((void*)0));
     }
}
...
fake_struct(Ident *ident)
{
   ...
   if (tok.type == IDENTIFIER || tok.type == MODIFIER || tok.type == QUALIFIER) {
        ...
        file_error(dcgettext (((void*)0), "missing `;' after struct declaration", 5), &tok);
   return 1;
     }
     return 0;
}
...
initializer_list()
{
   ...
   case RBRACE0:
      ...
      return;
        ...
        break;
   case 0:
        file_error(dcgettext (((void*)0), "unexpected end of file in initializer list", 5),
     ((void*)0));
        return;
   case ',':
        break;
...
skip_struct()
{
   ...
   if (skip_balanced('{', '}', 1) == -1) {
        file_error(dcgettext (((void*)0), "unexpected end of file in struct", 5), ((void*)0));
        return;
   }
     }
...
dcl(Ident *idptr)
{
   ...
   if (tok.type == MODIFIER) {
      ...
      idptr->type_end = curs-1;
        ...
        if (skip_balanced('(', ')', 0) == -1) {
      file_error(dcgettext (((void*)0), "unexpected end of file in function declaration", 5),
          ((void*)0));
      return 1;
        }
...
maybe_parm_list(int *parm_cnt_return)
{
   ...
   default:
        if (verbose)
      file_error(dcgettext (((void*)0), "unexpected token in parameter list", 5),
          &tok);
        level = 0;
        do {
      if (tok.type == '(')
    ...
    if (level-- == 0)
     ...
     if (verbose)
   file_error(dcgettext (((void*)0), "unexpected end of file in parameter list", 5), ((void*)0));
}
...
func_body()
{
   ...
   case RBRACE0:
        if (use_indentation) {
      if (verbose && level != 1)
    file_error(dcgettext (((void*)0), "forced function body close", 5), ((void*)0));
      for ( ; level; level--) {
    delete_autos(level);
      }
      break;
   ...
   case 0:
        if (verbose)
      file_error(dcgettext (((void*)0), "unexpected end of file in function body", 5),
          ((void*)0));
        caller = ((void*)0);
        return;
   }
...
declare(Ident *ident, int maybe_knr)
{
  ...
  || save_stack_is_empty()) {
   ...
   if (ident->storage == StaticStorage
      ...
      dcgettext (((void*)0), "%s/%d redefined", 5),
      ident->name, sp->arity);
        else
      error_at_line(0, 0, filename, ident->line,
      dcgettext (((void*)0), "%s redefined", 5),
      ident->name);
        error_at_line(0, 0, sp->source, sp->def_line,
        dcgettext (((void*)0), "this is the place of previous definition", 5));
   }
     }
     ...
     sp->level = level;
     if (debug)
   fprintf(stderr, dcgettext (((void*)0), "%s:%d: %s/%d defined to %s\n", 5),
   filename,
   line_num,
...
declare_type(Ident *ident)
{
   ...
   sp = install(ident->name, 0x04);
     ...
     sp->ref_line = ((void*)0);
     if (debug)
   fprintf(stderr, dcgettext (((void*)0), "%s:%d: type %s\n", 5), filename, line_num,
    ident->name);
}
