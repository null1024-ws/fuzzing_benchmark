# 1 "/benchmark/project/cflow-1.6/gnu/printf-parse.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/project/cflow-1.6/gnu/printf-parse.c" 2
# 33 "/benchmark/project/cflow-1.6/gnu/printf-parse.c"
# 1 "../config.h" 1
# 34 "/benchmark/project/cflow-1.6/gnu/printf-parse.c" 2
...
# 39 "/benchmark/project/cflow-1.6/gnu/printf-parse.c" 2
# 50 "/benchmark/project/cflow-1.6/gnu/printf-parse.c"
# 1 "/usr/local/lib/clang/12.0.0/include/stddef.h" 1 3
# 51 "/benchmark/project/cflow-1.6/gnu/printf-parse.c" 2
# 61 "/benchmark/project/cflow-1.6/gnu/printf-parse.c"
...
# 62 "/benchmark/project/cflow-1.6/gnu/printf-parse.c" 2
...
extern int _gl_cxxalias_dummy;
# 66 "/benchmark/project/cflow-1.6/gnu/printf-parse.c" 2
...
extern int _gl_cxxalias_dummy;
# 69 "/benchmark/project/cflow-1.6/gnu/printf-parse.c" 2
...
# 72 "/benchmark/project/cflow-1.6/gnu/printf-parse.c" 2
...
# 75 "/benchmark/project/cflow-1.6/gnu/printf-parse.c" 2
# 84 "/benchmark/project/cflow-1.6/gnu/printf-parse.c"
int
...
# 136 "/benchmark/project/cflow-1.6/gnu/printf-parse.c"
  while (*cp != '\0')
    {
      char c = *cp++;
      if (c == '%')
              ...
              if (*np == '$')
                {
                  ...
                  for (np = cp; *np >= '0' && *np <= '9'; np++)
                    n = xsum (((n) <= (18446744073709551615UL) / (10) ? (size_t) (n) * (10) : (18446744073709551615UL)), *np - '0');
                  if (n == 0)
          ...
          if (*cp == '*')
            {
              ...
              { size_t n = (dp->width_arg_index); if (n >= a_allocated) { size_t memory_size; argument *memory; a_allocated = ((a_allocated) <= (18446744073709551615UL) / (2) ? (size_t) (a_allocated) * (2) : (18446744073709551615UL)); if (a_allocated <= n) a_allocated = xsum (n, 1); memory_size = ((a_allocated) <= (18446744073709551615UL) / (sizeof (argument)) ? (size_t) (a_allocated) * (sizeof (argument)) : (18446744073709551615UL)); if (((memory_size) == (18446744073709551615UL))) goto out_of_memory; memory = (argument *) (a->arg != a->direct_alloc_arg ? realloc (a->arg, memory_size) : malloc (memory_size)); if (memory == ((void*)0)) goto out_of_memory; if (a->arg == a->direct_alloc_arg) memcpy (memory, a->arg, a->count * sizeof (argument)); a->arg = memory; } while (a->count <= n) a->arg[a->count++].type = TYPE_NONE; if (a->arg[n].type == TYPE_NONE) a->arg[n].type = (TYPE_INT); else if (a->arg[n].type != (TYPE_INT)) goto error; };
            }
          else if (*cp >= '0' && *cp <= '9')
            {
              size_t width_length;
...
# 443 "/benchmark/project/cflow-1.6/gnu/printf-parse.c"
                  else
                    break;
                }
              ...
              switch (c)
                ...
                case 'n':
                  ...
                  break;
# 558 "/benchmark/project/cflow-1.6/gnu/printf-parse.c"
                case '%':
                  type = TYPE_NONE;
                  break;
                default:
