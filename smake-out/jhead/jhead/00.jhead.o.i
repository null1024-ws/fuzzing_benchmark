# 1 "/benchmark/project/jhead-3.00/jhead.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/project/jhead-3.00/jhead.c" 2
# 18 "/benchmark/project/jhead-3.00/jhead.c"
# 1 "/benchmark/project/jhead-3.00/jhead.h" 1
...
# 10 "/benchmark/project/jhead-3.00/jhead.h" 2
...
# 11 "/benchmark/project/jhead-3.00/jhead.h" 2
...
extern char *stpncpy (char *__restrict __dest,
        const char *__restrict __src, size_t __n)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2)));
# 12 "/benchmark/project/jhead-3.00/jhead.h" 2
...
extern int timespec_get (struct timespec *__ts, int __base)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1)));
# 13 "/benchmark/project/jhead-3.00/jhead.h" 2
...
# 14 "/benchmark/project/jhead-3.00/jhead.h" 2
...
# 15 "/benchmark/project/jhead-3.00/jhead.h" 2
# 28 "/benchmark/project/jhead-3.00/jhead.h"
...
extern int utime (const char *__file,
    const struct utimbuf *__file_times)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1)));
# 29 "/benchmark/project/jhead-3.00/jhead.h" 2
...
# 31 "/benchmark/project/jhead-3.00/jhead.h" 2
...
# 33 "/benchmark/project/jhead-3.00/jhead.h" 2



typedef unsigned char uchar;
# 58 "/benchmark/project/jhead-3.00/jhead.h"
typedef struct {
    uchar * Data;
    int Type;
    unsigned Size;
...
# 192 "/benchmark/project/jhead-3.00/jhead.h"
...
# 210 "/benchmark/project/jhead-3.00/jhead.h"
...
extern int ShowTags;
# 19 "/benchmark/project/jhead-3.00/jhead.c" 2
...
# 21 "/benchmark/project/jhead-3.00/jhead.c" 2
# 35 "/benchmark/project/jhead-3.00/jhead.c"
...
static char * strftime_args = ((void*)0);
...
static char * ApplyCommand = ((void*)0);
static char * FilterModel = ((void*)0);
...
static unsigned FileTimeToExif = 0;
...
static char * ThumbSaveName = ((void*)0);


static char * ThumbInsertName = ((void*)0);


static int RegenThumbnail = 0;

static char * ExifXferScrFile = ((void*)0);


static int EditComment = 0;
static int SupressNonFatalErrors = 0;

static char * CommentSavefileName = ((void*)0);
static char * CommentInsertfileName = ((void*)0);
static char * CommentInsertLiteral = ((void*)0);

static int AutoRotate = 0;
static int ZeroRotateTagOnly = 0;

static int ShowFileInfo = 1;
# 113 "/benchmark/project/jhead-3.00/jhead.c"
void ErrFatal(const char * msg)
{
    fprintf(stderr,"\nError : %s\n", msg);
...
static int FileEditComment(char * TempFileName, char * Comment, int CommentSize)
{
...

    {
        ...
        if (strlen(Editor) > 4096) ErrFatal("env too long");
    ...
    CommentSize = fread(Comment, 1, 16000, file);
...
# 319 "/benchmark/project/jhead-3.00/jhead.c"
static int shellescape(char* to, const char* from)
{
    ...
    while(from[i])
    {
        ...
        if (j >= 4096) ErrFatal("max path exceeded");
    }
    to[j++] = '"';
    return j;
}
...
{
    int a,e;
    char ExecString[4096*3];
    char TempName[4096 +10];
    int TempUsed = 0;
    ...
    if (a || (*__errno_location ())){

        if ((*__errno_location ())) perror("system");
...
static int CheckFileSkip(void)
{
    ...
    if (FilterModel){
        ...
        if (strstr(ImageInfo.CameraModel, FilterModel) == ((void*)0)){
    ...
    if (ExifOnly){
        ...
        if (FindSection(0xE1) == ((void*)0)){
            return 1;
        }
    }

    if (PortraitOnly == 1){
        if (ImageInfo.Width > ImageInfo.Height) return 1;
    }
...
static void RelativeName(char * OutFileName, const char * NamePattern, const char * OrigName)
{
    ...
    if (Subst){
        ...
        strncat(OutFileName, OrigName, 4096);
        strncat(OutFileName, Subst+2, 4096);
    }else{
        strncpy(OutFileName, NamePattern, 4096);
    }
}
# 561 "/benchmark/project/jhead-3.00/jhead.c"
static void DoFileRenaming(const char * FileName)
{
    ...
    strncpy(NewBaseName, FileName, 4096);
    ...
    if (strftime_args){
        ...
        pattern[4096 -1]=0;
        strncpy(pattern, strftime_args, 4096 -1);
        while ((s = strstr(pattern, "%f")) && strlen(pattern) + n < 4096 -1){
            ...
            for (a=0;pattern[a];a++){
                if (pattern[a] == '%'){
                    ...
                    if (ppos >= 0 && a<ppos+4){
                        ...
                        if (ppos+nl+l+1 >= 4096) ErrFatal("str overflow");
                        memmove(pattern+ppos+nl, pattern+a+1, l+1);
                        memcpy(pattern+ppos, num, nl);
                        break;
                    }
                }else if (!((*__ctype_b_loc ())[(int) ((pattern[a]))] & (unsigned short int) _ISdigit)){
                    ppos = -1;
                }
            }
        }
        strftime(NewName, 4096, pattern, &tm);
    }else{
    ...
    AddLetter = ((*__ctype_b_loc ())[(int) ((NewBaseName[strlen(NewBaseName)-1]))] & (unsigned short int) _ISdigit);
    for (a=0;;a++){
        char NewName[4096 +10];
        char NameExtra[3];
        struct stat dummy;
    ...
    if (ImageInfo.Orientation != 1){
        ...
        if (!ZeroRotateTagOnly){
            ...
            if (ImageInfo.ThumbnailOffset &&
                ...
                char ThumbTempName_in[4096 +5];
                char ThumbTempName_out[4096 +5];
...
{
    char ThumbnailGenCommand[4096*2+50];
    if (ImageInfo.ThumbnailOffset == 0 || ImageInfo.ThumbnailAtEnd == 0){
...
static void ProcessFile(const char * FileName)
{
    ...
    if ((DoModify & 1) || RenameToDate || Exif2FileTime){
    ...
    strncpy(ImageInfo.FileName, FileName, 4096);
...
# 853 "/benchmark/project/jhead-3.00/jhead.c"
        if (CheckFileSkip()){
            DiscardData();
            return;
        }
    ...
    if (
        ...
        CommentSec = FindSection(0xFE);

        if (CommentSec == ((void*)0)){
            ...
            DummyData[1] = 2;
            DummyData[2] = 0;
            CommentSec = CreateSection(0xFE, DummyData, 2);
        }
        ...
        if (CommentInsertfileName){
            ...
            if (CommentFile == ((void*)0)){
                printf("Could not open '%s'\n",CommentFileName);
            }else{


                CommentSize = fread(Comment, 1, 16000, CommentFile);
                fclose(CommentFile);
                if (CommentSize < 0) CommentSize = 0;
            }
        }else if (CommentInsertLiteral){
            strncpy(Comment, CommentInsertLiteral, 16000);
            CommentSize = strlen(Comment);
        }else{
# 1050 "/benchmark/project/jhead-3.00/jhead.c"
            memcpy(Comment, (char *)CommentSec->Data+2, CommentSize);

            {
                char EditFileName[4096 +5];
...
skip_unixtime:
    ...
    if (DeleteComments){
        if (RemoveSectionType(0xFE)) Modified = 1;
    }
    if (DeleteExif){
        if (RemoveSectionType(0xE1)) Modified = 1;
    }
    if (DeleteIptc){
        if (RemoveSectionType(0xED)) Modified = 1;
    }
    if (DeleteXmp){
        if (RemoveSectionType(0x10E1)) Modified = 1;
    }
    if (DeleteUnknown){
        if (RemoveUnknownSections()) Modified = 1;
    }


    if (Modified){
        char BackupName[4096 +5];
        struct stat buf;
...
static void Usage (void)
{
    printf("Jhead is a program for manipulating settings and thumbnails in Exif jpeg headers\n"
           "used by most Digital Cameras.  v""3.00"" Matthias Wandel, Jan 30 2013.\n"
...
# 1400 "/benchmark/project/jhead-3.00/jhead.c"
           );
...
int main (int argc, char **argv)
{
    ...
    for (argn=1;argn<argc;argn++){
        ...
        if (!strcmp(arg,"-te")){
            ...
            printf("Jhead version: ""3.00""   Compiled: ""Feb 19 2025""\n");
            exit(0);
            ...
            for (a=0;arg[a+3];a++){
                if (((*__ctype_b_loc ())[(int) ((DateSet[a]))] & (unsigned short int) _ISdigit)){
                    if (!((*__ctype_b_loc ())[(int) ((arg[a+3]))] & (unsigned short int) _ISdigit)){
                        a = 0;
                        break;
                    }
                }else{
            ...
            DoModify |= 1;
# 1674 "/benchmark/project/jhead-3.00/jhead.c"
        }else{
    ...
    if (ThumbSaveName != ((void*)0) && strcmp(ThumbSaveName, "&i") == 0){
    ...
    if (EditComment){
        if (CommentSavefileName != ((void*)0) || CommentInsertfileName != ((void*)0)){
    ...
    for (;argn<argc;argn++){
        FilesMatched = 0;
# 1730 "/benchmark/project/jhead-3.00/jhead.c"
